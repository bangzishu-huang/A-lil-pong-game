...
class Paddle(pygame.sprite.Sprite):
    def __init__(self, groups):
        ...
        pygame.draw.rect(self.shadow_surface, COLORS['paddle shadow'], pygame.FRect((0,0), SIZE['paddle']), 0, 6)


        # rect and movement
        self.rect = self.image.get_frect(center = POSITION['player'])
...
class Opp(Paddle):
    def __init__(self, groups, ball):
        super().__init__(groups)
        self.speed = SPEED['opp']
        self.rect.center = POSITION['opp']
        self.ball = ball
...
class Ball(pygame.sprite.Sprite):
    def __init__(self, groups, paddle_sprites, update_score):
        ...
        # self.image.fill(COLORS['ball'])  # fill for rectangular ball
    ...
    def wall_collisions(self):
        ...
        if self.rect.right >= WINDOW_WIDTH or self.rect.left <= 0:
            self.update_score('player' if self.rect.x < WINDOW_WIDTH / 2 else 'opp')
            self.reset()
    ...
    def reset(self):
        ...
        # # testing collision in enclosed space
